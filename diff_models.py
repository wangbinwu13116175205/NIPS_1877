...
class Guide_diff(nn.Module):
    def __init__(self, config, tem_size,inputdim=1, target_dim=36, is_itp=False):
        super().__init__()
        ...
        self.node_emb=nn.Parameter(torch.empty(node_num, self.channels))
    ...
    def forward(self, x, side_info, diffusion_step, itp_x, cond_mask):
        ...
        node_embe=self.node_emb.expand(B, L, *self.node_emb.shape)
        x=x+node_embe.transpose(3,1)
        if self.is_itp:
...
class NoiseProject(nn.Module):
    def __init__(self, side_dim, channels, diffusion_embedding_dim, nheads, target_dim, proj_t, tem_size,node_size,order=2, include_self=True,
        ...
        self.forward_feature = SpatialLearning(channels=channels, nheads=nheads, target_dim=target_dim,
                                               ...
                                               adj_file=adj_file, proj_t=proj_t, is_cross=is_cross_s,node_size=node_size)
        self.temporal_attention = TATT(c_in=channels,num_nodes=node_size,tem_size=tem_size)
    def forward(self, x, side_info, diffusion_emb, itp_info, support):
        ...
        #node_embed=self.node_emb.expand(B, self.in_steps, *self.node_emb.shape

        y = x + diffusion_emb
        y = self.forward_time(y, base_shape, itp_info)
        y=y.reshape(B,-1,K,L)
        atten_temp=self.temporal_attention(y)
        atten_temp= atten_temp.transpose(-1,-2)
        y=torch.einsum('bcnl,blq->bcnq',y,atten_temp)+y   
        y=y.reshape(B,channel,K*L)
        y = self.forward_feature(y, base_shape, support, itp_info) 

...
class PriSTI(nn.Module):
    def __init__(self, target_dim, seq_len, config, device):
        ...
        input_dim = 2
        self.diffmodel = Guide_diff(config_diff, seq_len,input_dim, target_dim, self.use_guide)
...
class PriSTI_Pems04(PriSTI):
    def __init__(self, config, device, target_dim=307, seq_len=48):
        super(PriSTI_Pems04, self).__init__(target_dim, seq_len, config, device)
        self.config = config

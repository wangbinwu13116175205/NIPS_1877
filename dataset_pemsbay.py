...
class PemsBAY_Dataset(Dataset):
    def __init__(self, eval_length=24, mode="train", val_len=0.1, test_len=0.2, missing_pattern='block',
        ...
        if missing_pattern == 'block':
            eval_mask = sample_mask(shape=(52116, 325), p=0.0015, p_noise=rate, min_seq=12, max_seq=12 * 4, rng=self.rng)
        elif missing_pattern == 'point':
            eval_mask = sample_mask(shape=(52116, 325), p=0., p_noise=rate, max_seq=12, min_seq=12 * 4, rng=self.rng)

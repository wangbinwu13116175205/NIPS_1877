...
class Pems04_Dataset(Dataset):
    def __init__(self, eval_length=48, mode="train", val_len=0.1, test_len=0.2, missing_pattern='point',
                 is_interpolate=False, target_strategy='random',rate=0.2):
        ...
        self.train_mean=df.mean(axis=0)
        self.train_std=df.var(axis=0)
        SEED = 9101112
        self.rng = np.random.default_rng(SEED)
        if missing_pattern == 'block':
            eval_mask = sample_mask(shape=(16992, 307), p=0.0015, p_noise=rate, min_seq=12, max_seq=12 * 4, rng=self.rng)
        elif missing_pattern == 'point':
            eval_mask = sample_mask(shape=(16992, 307), p=0., p_noise=rate, max_seq=12, min_seq=12 * 4, rng=self.rng)
        gt_mask = (1-(eval_mask | (1-ob_mask))).astype('uint8')
        val_start = int((1 - val_len - test_len) * len(df))
        test_start = int((1 - test_len) * len(df))
        df[np.isnan(df)] = 0
        c_data = (
             (df- self.train_mean) / self.train_std
        ) * ob_mask
        if mode == 'train':
...
def get_dataloader(batch_size, device, val_len=0.1, test_len=0.2, missing_pattern='block',
                   is_interpolate=False, num_workers=4, target_strategy='random',rate=0.2):
    dataset = Pems04_Dataset(mode="train", val_len=val_len, test_len=test_len, missing_pattern=missing_pattern,
                             is_interpolate=is_interpolate, target_strategy=target_strategy,rate=rate)

...
class SpatialLearning(nn.Module):
    def __init__(self, channels, nheads, target_dim, order, include_self, device, is_adp, adj_file, proj_t, is_cross,node_size):
        super().__init__()
        self.is_cross = is_cross
        self.feature_layer = SpaDependLearning(channels, nheads=nheads, order=order, target_dim=target_dim,
                                      ...
                                      proj_t=proj_t, node_size=node_size,is_cross=is_cross)
...
class SpaDependLearning(nn.Module):
    def __init__(self, channels, nheads, target_dim, order, include_self, device, is_adp, adj_file, proj_t, node_size,is_cross=True):
        super().__init__()
        self.is_cross = is_cross
        ...
        self.W1=nn.Parameter(torch.rand(channels, node_size), requires_grad=True)
        torch.nn.init.xavier_uniform_(self.W1)
        self.W2=nn.Parameter(torch.rand(node_size, channels), requires_grad=True)
        torch.nn.init.xavier_uniform_(self.W2)
        self.atten='Ex_attention'
    def forward(self, y, base_shape, support, itp_y=None):
        B, channel, K, L = base_shape
        y_in1 = y
        ...
        elif self.atten=='Ex_attention':
            y_attn = y.reshape(B, channel, K, L).permute(0, 3, 2, 1).reshape(B * L,  K,channel)# [B*L, C, K]
            y_e=torch.einsum('bnk,kd->bnd', y_attn, self.W1)# [B*L, K, K]
            ex_attn=y_e.softmax(dim=-1)# [B*L, K, K]
            y_out=torch.einsum('bnk,kd->bnd', ex_attn, self.W2)# [B*L, K, C]
            y_attn=y_out.reshape(B, L, K, channel).permute(0,3, 2, 1).reshape(B, channel, K * L)
        y_attn = y_in1 + y_attn
